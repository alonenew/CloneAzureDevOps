...
function Sidebar() {
  ...
  const Menus = [
    ...
    { title: "Board", src:"https://cdn.vsassets.io/ext/ms.vss-work-web/common-content/Content/Nav-Plan.XB8qU6.png" , gap: true },
  ...
  return (
    <>
      {token && <Box className={` ${open ? " w-65" : "w-12"} duration-400`}>
        <Box className={`relative flex flex-col justify-between min-h-[calc(100vh-48px)] h-full border border-gray-300 overflow-auto shadow bg-gray-200 `}
        >
          <Box className={`w-full ${open ? "flex items-center py-1 pr-2" : ""}`}>
            <Box className={`flex items-center duration-200 ${!open ? "mx-auto" : "mr-auto"}`}> 
              <img src={`https://www.svgrepo.com/show/349488/react.svg`} alt="" className='rounded-md w-6 h-6 m-2.5' /> 
              <Typography variant='body1' className={`${!open && "hidden"} font-bold duration-200`} >
                Miniproject
              </Typography>
            </Box>
              ...
              {Menus.map((Menu, index) => (
                <Link to={Menu.link} className={`py-1 flex justify-center items-center rounded-md hover:bg-gray-300 `} key={index}>
                  <li className={`text-sm flex items-center duration-200 ${!open ? "mx-auto" : "mr-auto"}`} >
                    ...
                    <Typography variant='body2' className={`${!open && "hidden"} duration-200`} >
                      {Menu.title}
                    </Typography>
                  </li>
                  <hr />
            ...
            <Box className={`w-full ${open ? "flex items-center py-2 pr-2" : ""}`}>
              <Box className={`flex items-center duration-200 `}> 
